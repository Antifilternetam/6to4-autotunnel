...
setup_tunnel() {
  ...
  sudo modprobe ipv6
  sudo ip tunnel add $TUN_IF mode sit remote any local "$MY_IPV4" ttl 255 2>/dev/null || true
  sudo ip link set $TUN_IF up
  sudo ip -6 addr flush dev $TUN_IF 2>/dev/null
...
remove_all_tunnels() {
  ...
  for iface in $(ip tunnel show | grep '^t6t' | awk '{print $1}'); do
    ...
    sudo ip tunnel del "$iface" 2>/dev/null
...
setup_rathole() {
  ...
  if [[ "$ROLE" == "iran" ]]; then
    ...
    echo -e "${CYAN}✅ لطفاً گزینه 'y' را وارد کنید تا تونل با IPv6 ساخته شود.${NC}"
    echo -e "\n${GREEN}اگر آماده‌اید، Enter بزنید تا نصب رتهول آغاز شود...${NC}"
    read
    ...
    echo -e "\n${GREEN}🛰️ آدرس‌های اتصال به سرور ایران:${NC}"
    echo -e "🌐 IPv6:  ${YELLOW}$IRAN_IPV6${NC}"
    echo -e "${CYAN}✅ از این آدرس‌ها هنگام راه‌اندازی رتهول استفاده کنید.${NC}"
    echo -e "${GREEN}Enter بزنید تا وارد منوی رتهول شوید...${NC}"
    read
    bash <(curl -Ls --ipv4 https://raw.githubusercontent.com/Musixal/rathole-tunnel/main/rathole_v2.sh)
