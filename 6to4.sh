...
setup_tunnel() {
  ...
  sudo modprobe ipv6
  sudo ip tunnel del $TUN_IF 2>/dev/null
...
remove_all_tunnels() {
  echo -e "${YELLOW}🧹 Removing all 6to4 tunnels...${NC}"
  for tun in $(ip tunnel show | grep '^t6t' | awk '{print $1}'); do
    sudo ip -6 addr flush dev "$tun" 2>/dev/null
  ...
  echo -e "\n${GREEN}✅ All tunnels and firewall rules cleaned.${NC}"
  echo -e "${CYAN}برای بازگشت به منو Enter بزنید...${NC}"
  read
}
...
setup_rathole() {
  ...
  if [[ "$ROLE" == "iran" ]]; then
    ...
    echo -e "${CYAN}✅ لطفاً در آن مرحله فقط گزینه 'y' را وارد کنید تا تونل با IPv6 ساخته شود.${NC}"
    ...
    echo -e "\n${GREEN}🛰️ توجه: برای اتصال به سرور ایران، از یکی از آدرس‌های زیر استفاده کنید:${NC}"
    echo -e "${YELLOW}🔹 IPv6:  $IRAN_IPV6${NC}"
    echo -e "${CYAN}✅ در رتهول، آدرس بالا را به عنوان سرور ایران وارد کن.${NC}"
    echo -e "${CYAN}اگر آماده‌ای، Enter بزن تا وارد رتهول شوی...${NC}"
    read
    bash <(curl -Ls --ipv4 https://raw.githubusercontent.com/Musixal/rathole-tunnel/main/rathole_v2.sh)
  else
    echo -e "${RED}[!] نقش یا IP سرور ایران مشخص نیست. لطفاً ابتدا تونل 6to4 را بسازید.${NC}"
  fi
}
